...
@section('content')
    <div class="nxl-container">
        ...
        <div class="page-header d-flex justify-content-between align-items-center">
            <div>
                <h4 class="fw-bold mb-1">Create Approval Process</h4>
                <p class="text-muted mb-0">
                    Add a new step approval process linked to governance
                </p>
            </div>
        ...
        <div class="card shadow-sm border-0 mt-3">
            <div class="card-body">
                <form method="POST" action="{{ route('procurement.settings.step-approvals.store') }}">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" class="form-control"
                            placeholder="e.g. Director Approval, Committee Review" value="{{ old('name') }}" required>
                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="step_stage_id" class="form-select">
                                ...
                                @foreach($stepStages as $stepStage)
                                    <option value="{{ $stepStage->id }}" {{ old('step_stage_id') == $stepStage->id ? 'selected' : '' }}>
                                        {{ $stepStage->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="governance_node_id" class="form-select">
                                ...
                                @foreach($governanceNodes as $node)
                                    <option value="{{ $node->id }}" {{ old('governance_node_id') == $node->id ? 'selected' : '' }}>
                                        {{ $node->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" rows="3" class="form-control"
                            placeholder="Brief description of this approval process">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="number" name="approval_order" class="form-control"
                                placeholder="e.g. 1, 2, 3" value="{{ old('approval_order', 0) }}" min="0">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <button class="btn btn-primary">
                            <i class="feather-save me-1"></i> Save Approval Process
                        </button>
                    </div>
                </form>
            </div>
